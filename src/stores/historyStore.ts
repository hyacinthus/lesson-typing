...
interface HistoryStore {
  ...
  getAllRecentPracticeLogs: (limit?: number) => Promise<PracticeRecord[]>;
  clearAll: () => void;
}
...
export const useHistoryStore = create<HistoryStore>()(
  persist(
    (set, get) => ({
      ...
      getAllRecentPracticeLogs: async (limit = 20) => {
        const { user } = useAuthStore.getState();
        if (!user) return [];
        ...
        try {
          ...
          if (error) {
            console.error('Failed to fetch all recent practice logs:', error);
            return [];
          }
          ...
          console.error('Error fetching all recent practice logs:', err);
          return [];
        }
      },
