# Lyrics Curation Progress

Tracking the review status of lyrics lessons across all 9 languages, following `lyrics-curation.md`.

| Language   | Status      | Notes                        |
|------------|-------------|------------------------------|
| Chinese    | Done        | Curated in v0.15.0           |
| English    | Done        | Curated in v0.15.2           |
| French     | Done        | Curated in v0.15.2           |
| German     | Done        | Curated in v0.15.2           |
| Italian    | Done        | Curated in v0.15.2           |
| Japanese   | Done        | Curated in v0.15.3           |
| Korean     | Done        | Curated in v0.15.4           |
| Portuguese | Done        | Curated in v0.15.4           |
| Spanish    | Pending     |                              |
